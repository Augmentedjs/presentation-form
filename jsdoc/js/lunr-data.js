window.lunrData = {
  "index": {
    "version": "1.0.0",
    "fields": [
      {
        "name": "longname",
        "boost": 1000
      },
      {
        "name": "name",
        "boost": 500
      },
      {
        "name": "tags",
        "boost": 300
      },
      {
        "name": "kind",
        "boost": 110
      },
      {
        "name": "title",
        "boost": 100
      },
      {
        "name": "summary",
        "boost": 70
      },
      {
        "name": "description",
        "boost": 50
      },
      {
        "name": "body",
        "boost": 1
      }
    ],
    "ref": "id",
    "tokenizer": "default",
    "documentStore": {
      "store": {
        "index.html": [
          "augmented.j",
          "develop",
          "index",
          "next",
          "present",
          "readm",
          "simpli"
        ],
        "global.html": [
          "document",
          "global"
        ],
        "list_class.html": [
          "class",
          "document",
          "list",
          "list:class"
        ],
        "AutomaticForm.html": [
          "add",
          "addit",
          "array",
          "automat",
          "automaticform",
          "bind",
          "button",
          "class",
          "cor",
          "creat",
          "crossorigin",
          "data",
          "descript",
          "display",
          "dom",
          "element",
          "fetch",
          "field",
          "form",
          "function",
          "hidden",
          "json",
          "legaci",
          "legci",
          "markup",
          "model",
          "name",
          "nestedinput",
          "option",
          "other",
          "prepopul",
          "render",
          "reset",
          "resetbutton",
          "schema",
          "set",
          "submit",
          "submitbutton",
          "support",
          "titl",
          "ui",
          "uri",
          "us",
          "valid",
          "view"
        ],
        "AutomaticForm.html#_retrieveSchema": [
          "_retrieveschema",
          "automaticform#_retrieveschema",
          "function"
        ],
        "AutomaticForm.html#setURI": [
          "automaticform#seturi",
          "function",
          "set",
          "seturi",
          "uri"
        ],
        "AutomaticForm.html#setSchema": [
          "automaticform#setschema",
          "function",
          "schema",
          "set",
          "setschema"
        ],
        "AutomaticForm.html#showProgressBar": [
          "automaticform#showprogressbar",
          "bar",
          "enable/dis",
          "function",
          "progress",
          "show",
          "showprogressbar"
        ],
        "AutomaticForm.html#showMessage": [
          "automaticform#showmessag",
          "form",
          "function",
          "messag",
          "relat",
          "show",
          "showmessag"
        ],
        "AutomaticForm.html#validate": [
          "automaticform#valid",
          "form",
          "function",
          "valid"
        ],
        "AutomaticForm.html#isValid": [
          "automaticform#isvalid",
          "form",
          "function",
          "isvalid",
          "valid"
        ],
        "AutomaticForm.html#render": [
          "automaticform#rend",
          "form",
          "function",
          "render"
        ],
        "AutomaticForm.html#reset": [
          "automaticform#reset",
          "form",
          "function",
          "reset"
        ],
        "AutomaticForm.html#populate": [
          "automaticform#popul",
          "data",
          "form",
          "function",
          "popul"
        ],
        "AutomaticForm.html#remove": [
          "automaticform#remov",
          "bind",
          "form",
          "function",
          "remov"
        ]
      },
      "length": 15
    },
    "tokenStore": {
      "root": {
        "docs": {},
        "a": {
          "docs": {},
          "u": {
            "docs": {},
            "g": {
              "docs": {},
              "m": {
                "docs": {},
                "e": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "t": {
                      "docs": {},
                      "e": {
                        "docs": {},
                        "d": {
                          "docs": {},
                          ".": {
                            "docs": {},
                            "j": {
                              "docs": {
                                "index.html": {
                                  "ref": "index.html",
                                  "tf": 214
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "o": {
                "docs": {},
                "m": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "AutomaticForm.html": {
                          "ref": "AutomaticForm.html",
                          "tf": 0.7936507936507936
                        }
                      },
                      "i": {
                        "docs": {},
                        "c": {
                          "docs": {},
                          "f": {
                            "docs": {},
                            "o": {
                              "docs": {},
                              "r": {
                                "docs": {},
                                "m": {
                                  "docs": {
                                    "AutomaticForm.html": {
                                      "ref": "AutomaticForm.html",
                                      "tf": 1900
                                    }
                                  },
                                  "#": {
                                    "docs": {},
                                    "_": {
                                      "docs": {},
                                      "r": {
                                        "docs": {},
                                        "e": {
                                          "docs": {},
                                          "t": {
                                            "docs": {},
                                            "r": {
                                              "docs": {},
                                              "i": {
                                                "docs": {},
                                                "e": {
                                                  "docs": {},
                                                  "v": {
                                                    "docs": {},
                                                    "e": {
                                                      "docs": {},
                                                      "s": {
                                                        "docs": {},
                                                        "c": {
                                                          "docs": {},
                                                          "h": {
                                                            "docs": {},
                                                            "e": {
                                                              "docs": {},
                                                              "m": {
                                                                "docs": {},
                                                                "a": {
                                                                  "docs": {
                                                                    "AutomaticForm.html#_retrieveSchema": {
                                                                      "ref": "AutomaticForm.html#_retrieveSchema",
                                                                      "tf": 1150
                                                                    }
                                                                  }
                                                                }
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    },
                                    "s": {
                                      "docs": {},
                                      "e": {
                                        "docs": {},
                                        "t": {
                                          "docs": {},
                                          "u": {
                                            "docs": {},
                                            "r": {
                                              "docs": {},
                                              "i": {
                                                "docs": {
                                                  "AutomaticForm.html#setURI": {
                                                    "ref": "AutomaticForm.html#setURI",
                                                    "tf": 1150
                                                  }
                                                }
                                              }
                                            }
                                          },
                                          "s": {
                                            "docs": {},
                                            "c": {
                                              "docs": {},
                                              "h": {
                                                "docs": {},
                                                "e": {
                                                  "docs": {},
                                                  "m": {
                                                    "docs": {},
                                                    "a": {
                                                      "docs": {
                                                        "AutomaticForm.html#setSchema": {
                                                          "ref": "AutomaticForm.html#setSchema",
                                                          "tf": 1150
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      },
                                      "h": {
                                        "docs": {},
                                        "o": {
                                          "docs": {},
                                          "w": {
                                            "docs": {},
                                            "p": {
                                              "docs": {},
                                              "r": {
                                                "docs": {},
                                                "o": {
                                                  "docs": {},
                                                  "g": {
                                                    "docs": {},
                                                    "r": {
                                                      "docs": {},
                                                      "e": {
                                                        "docs": {},
                                                        "s": {
                                                          "docs": {},
                                                          "s": {
                                                            "docs": {},
                                                            "b": {
                                                              "docs": {},
                                                              "a": {
                                                                "docs": {},
                                                                "r": {
                                                                  "docs": {
                                                                    "AutomaticForm.html#showProgressBar": {
                                                                      "ref": "AutomaticForm.html#showProgressBar",
                                                                      "tf": 1150
                                                                    }
                                                                  }
                                                                }
                                                              }
                                                            }
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            },
                                            "m": {
                                              "docs": {},
                                              "e": {
                                                "docs": {},
                                                "s": {
                                                  "docs": {},
                                                  "s": {
                                                    "docs": {},
                                                    "a": {
                                                      "docs": {},
                                                      "g": {
                                                        "docs": {
                                                          "AutomaticForm.html#showMessage": {
                                                            "ref": "AutomaticForm.html#showMessage",
                                                            "tf": 1150
                                                          }
                                                        }
                                                      }
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    },
                                    "v": {
                                      "docs": {},
                                      "a": {
                                        "docs": {},
                                        "l": {
                                          "docs": {},
                                          "i": {
                                            "docs": {},
                                            "d": {
                                              "docs": {
                                                "AutomaticForm.html#validate": {
                                                  "ref": "AutomaticForm.html#validate",
                                                  "tf": 1150
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    },
                                    "i": {
                                      "docs": {},
                                      "s": {
                                        "docs": {},
                                        "v": {
                                          "docs": {},
                                          "a": {
                                            "docs": {},
                                            "l": {
                                              "docs": {},
                                              "i": {
                                                "docs": {},
                                                "d": {
                                                  "docs": {
                                                    "AutomaticForm.html#isValid": {
                                                      "ref": "AutomaticForm.html#isValid",
                                                      "tf": 1150
                                                    }
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    },
                                    "r": {
                                      "docs": {},
                                      "e": {
                                        "docs": {},
                                        "n": {
                                          "docs": {},
                                          "d": {
                                            "docs": {
                                              "AutomaticForm.html#render": {
                                                "ref": "AutomaticForm.html#render",
                                                "tf": 1150
                                              }
                                            }
                                          }
                                        },
                                        "s": {
                                          "docs": {},
                                          "e": {
                                            "docs": {},
                                            "t": {
                                              "docs": {
                                                "AutomaticForm.html#reset": {
                                                  "ref": "AutomaticForm.html#reset",
                                                  "tf": 1150
                                                }
                                              }
                                            }
                                          }
                                        },
                                        "m": {
                                          "docs": {},
                                          "o": {
                                            "docs": {},
                                            "v": {
                                              "docs": {
                                                "AutomaticForm.html#remove": {
                                                  "ref": "AutomaticForm.html#remove",
                                                  "tf": 1150
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    },
                                    "p": {
                                      "docs": {},
                                      "o": {
                                        "docs": {},
                                        "p": {
                                          "docs": {},
                                          "u": {
                                            "docs": {},
                                            "l": {
                                              "docs": {
                                                "AutomaticForm.html#populate": {
                                                  "ref": "AutomaticForm.html#populate",
                                                  "tf": 1150
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "d": {
            "docs": {},
            "d": {
              "docs": {
                "AutomaticForm.html": {
                  "ref": "AutomaticForm.html",
                  "tf": 0.7936507936507936
                }
              },
              "i": {
                "docs": {},
                "t": {
                  "docs": {
                    "AutomaticForm.html": {
                      "ref": "AutomaticForm.html",
                      "tf": 0.7936507936507936
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "r": {
              "docs": {},
              "a": {
                "docs": {},
                "y": {
                  "docs": {
                    "AutomaticForm.html": {
                      "ref": "AutomaticForm.html",
                      "tf": 0.7936507936507936
                    }
                  }
                }
              }
            }
          }
        },
        "d": {
          "docs": {},
          "e": {
            "docs": {},
            "v": {
              "docs": {},
              "e": {
                "docs": {},
                "l": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "p": {
                      "docs": {
                        "index.html": {
                          "ref": "index.html",
                          "tf": 14
                        }
                      }
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "c": {
                "docs": {},
                "r": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "p": {
                      "docs": {},
                      "t": {
                        "docs": {
                          "AutomaticForm.html": {
                            "ref": "AutomaticForm.html",
                            "tf": 0.7936507936507936
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "c": {
              "docs": {},
              "u": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "n": {
                      "docs": {},
                      "t": {
                        "docs": {
                          "global.html": {
                            "ref": "global.html",
                            "tf": 35
                          },
                          "list_class.html": {
                            "ref": "list_class.html",
                            "tf": 35
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "m": {
              "docs": {
                "AutomaticForm.html": {
                  "ref": "AutomaticForm.html",
                  "tf": 0.7936507936507936
                }
              }
            }
          },
          "a": {
            "docs": {},
            "t": {
              "docs": {},
              "a": {
                "docs": {
                  "AutomaticForm.html": {
                    "ref": "AutomaticForm.html",
                    "tf": 2.380952380952381
                  },
                  "AutomaticForm.html#populate": {
                    "ref": "AutomaticForm.html#populate",
                    "tf": 50
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "s": {
              "docs": {},
              "p": {
                "docs": {},
                "l": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "y": {
                      "docs": {
                        "AutomaticForm.html": {
                          "ref": "AutomaticForm.html",
                          "tf": 1.5873015873015872
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "i": {
          "docs": {},
          "n": {
            "docs": {},
            "d": {
              "docs": {},
              "e": {
                "docs": {},
                "x": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 1300
                    }
                  }
                }
              }
            }
          },
          "s": {
            "docs": {},
            "v": {
              "docs": {},
              "a": {
                "docs": {},
                "l": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "d": {
                      "docs": {
                        "AutomaticForm.html#isValid": {
                          "ref": "AutomaticForm.html#isValid",
                          "tf": 750
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "n": {
          "docs": {},
          "e": {
            "docs": {},
            "x": {
              "docs": {},
              "t": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 214
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "t": {
                "docs": {},
                "e": {
                  "docs": {},
                  "d": {
                    "docs": {},
                    "i": {
                      "docs": {},
                      "n": {
                        "docs": {},
                        "p": {
                          "docs": {},
                          "u": {
                            "docs": {},
                            "t": {
                              "docs": {
                                "AutomaticForm.html": {
                                  "ref": "AutomaticForm.html",
                                  "tf": 0.7936507936507936
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "m": {
              "docs": {},
              "e": {
                "docs": {
                  "AutomaticForm.html": {
                    "ref": "AutomaticForm.html",
                    "tf": 1.5873015873015872
                  }
                }
              }
            }
          }
        },
        "p": {
          "docs": {},
          "r": {
            "docs": {},
            "e": {
              "docs": {},
              "s": {
                "docs": {},
                "e": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "index.html": {
                          "ref": "index.html",
                          "tf": 214
                        }
                      }
                    }
                  }
                }
              },
              "p": {
                "docs": {},
                "o": {
                  "docs": {},
                  "p": {
                    "docs": {},
                    "u": {
                      "docs": {},
                      "l": {
                        "docs": {
                          "AutomaticForm.html": {
                            "ref": "AutomaticForm.html",
                            "tf": 0.7936507936507936
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "o": {
              "docs": {},
              "g": {
                "docs": {},
                "r": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "s": {
                      "docs": {},
                      "s": {
                        "docs": {
                          "AutomaticForm.html#showProgressBar": {
                            "ref": "AutomaticForm.html#showProgressBar",
                            "tf": 16.666666666666664
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "p": {
              "docs": {},
              "u": {
                "docs": {},
                "l": {
                  "docs": {
                    "AutomaticForm.html#populate": {
                      "ref": "AutomaticForm.html#populate",
                      "tf": 725
                    }
                  }
                }
              }
            }
          }
        },
        "r": {
          "docs": {},
          "e": {
            "docs": {},
            "a": {
              "docs": {},
              "d": {
                "docs": {},
                "m": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 110
                    }
                  }
                }
              }
            },
            "n": {
              "docs": {},
              "d": {
                "docs": {},
                "e": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "AutomaticForm.html": {
                        "ref": "AutomaticForm.html",
                        "tf": 0.7936507936507936
                      },
                      "AutomaticForm.html#render": {
                        "ref": "AutomaticForm.html#render",
                        "tf": 775
                      }
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "e": {
                "docs": {},
                "t": {
                  "docs": {
                    "AutomaticForm.html": {
                      "ref": "AutomaticForm.html",
                      "tf": 1.5873015873015872
                    },
                    "AutomaticForm.html#reset": {
                      "ref": "AutomaticForm.html#reset",
                      "tf": 775
                    }
                  },
                  "b": {
                    "docs": {},
                    "u": {
                      "docs": {},
                      "t": {
                        "docs": {},
                        "t": {
                          "docs": {},
                          "o": {
                            "docs": {},
                            "n": {
                              "docs": {
                                "AutomaticForm.html": {
                                  "ref": "AutomaticForm.html",
                                  "tf": 0.7936507936507936
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "l": {
              "docs": {},
              "a": {
                "docs": {},
                "t": {
                  "docs": {
                    "AutomaticForm.html#showMessage": {
                      "ref": "AutomaticForm.html#showMessage",
                      "tf": 12.5
                    }
                  }
                }
              }
            },
            "m": {
              "docs": {},
              "o": {
                "docs": {},
                "v": {
                  "docs": {
                    "AutomaticForm.html#remove": {
                      "ref": "AutomaticForm.html#remove",
                      "tf": 766.6666666666666
                    }
                  }
                }
              }
            }
          }
        },
        "s": {
          "docs": {},
          "i": {
            "docs": {},
            "m": {
              "docs": {},
              "p": {
                "docs": {},
                "l": {
                  "docs": {},
                  "i": {
                    "docs": {
                      "index.html": {
                        "ref": "index.html",
                        "tf": 14
                      }
                    }
                  }
                }
              }
            }
          },
          "c": {
            "docs": {},
            "h": {
              "docs": {},
              "e": {
                "docs": {},
                "m": {
                  "docs": {},
                  "a": {
                    "docs": {
                      "AutomaticForm.html": {
                        "ref": "AutomaticForm.html",
                        "tf": 2.380952380952381
                      },
                      "AutomaticForm.html#setSchema": {
                        "ref": "AutomaticForm.html#setSchema",
                        "tf": 75
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "t": {
              "docs": {
                "AutomaticForm.html": {
                  "ref": "AutomaticForm.html",
                  "tf": 3.1746031746031744
                },
                "AutomaticForm.html#setURI": {
                  "ref": "AutomaticForm.html#setURI",
                  "tf": 25
                },
                "AutomaticForm.html#setSchema": {
                  "ref": "AutomaticForm.html#setSchema",
                  "tf": 25
                }
              },
              "u": {
                "docs": {},
                "r": {
                  "docs": {},
                  "i": {
                    "docs": {
                      "AutomaticForm.html#setURI": {
                        "ref": "AutomaticForm.html#setURI",
                        "tf": 700
                      }
                    }
                  }
                }
              },
              "s": {
                "docs": {},
                "c": {
                  "docs": {},
                  "h": {
                    "docs": {},
                    "e": {
                      "docs": {},
                      "m": {
                        "docs": {},
                        "a": {
                          "docs": {
                            "AutomaticForm.html#setSchema": {
                              "ref": "AutomaticForm.html#setSchema",
                              "tf": 700
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "b": {
              "docs": {},
              "m": {
                "docs": {},
                "i": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "AutomaticForm.html": {
                        "ref": "AutomaticForm.html",
                        "tf": 1.5873015873015872
                      }
                    },
                    "b": {
                      "docs": {},
                      "u": {
                        "docs": {},
                        "t": {
                          "docs": {},
                          "t": {
                            "docs": {},
                            "o": {
                              "docs": {},
                              "n": {
                                "docs": {
                                  "AutomaticForm.html": {
                                    "ref": "AutomaticForm.html",
                                    "tf": 0.7936507936507936
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "p": {
              "docs": {},
              "p": {
                "docs": {},
                "o": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "AutomaticForm.html": {
                          "ref": "AutomaticForm.html",
                          "tf": 0.7936507936507936
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "h": {
            "docs": {},
            "o": {
              "docs": {},
              "w": {
                "docs": {
                  "AutomaticForm.html#showProgressBar": {
                    "ref": "AutomaticForm.html#showProgressBar",
                    "tf": 50
                  },
                  "AutomaticForm.html#showMessage": {
                    "ref": "AutomaticForm.html#showMessage",
                    "tf": 12.5
                  }
                },
                "p": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "g": {
                        "docs": {},
                        "r": {
                          "docs": {},
                          "e": {
                            "docs": {},
                            "s": {
                              "docs": {},
                              "s": {
                                "docs": {},
                                "b": {
                                  "docs": {},
                                  "a": {
                                    "docs": {},
                                    "r": {
                                      "docs": {
                                        "AutomaticForm.html#showProgressBar": {
                                          "ref": "AutomaticForm.html#showProgressBar",
                                          "tf": 700
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                },
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "s": {
                      "docs": {},
                      "s": {
                        "docs": {},
                        "a": {
                          "docs": {},
                          "g": {
                            "docs": {
                              "AutomaticForm.html#showMessage": {
                                "ref": "AutomaticForm.html#showMessage",
                                "tf": 700
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "g": {
          "docs": {},
          "l": {
            "docs": {},
            "o": {
              "docs": {},
              "b": {
                "docs": {},
                "a": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "global.html": {
                        "ref": "global.html",
                        "tf": 2045
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "c": {
          "docs": {},
          "l": {
            "docs": {},
            "a": {
              "docs": {},
              "s": {
                "docs": {},
                "s": {
                  "docs": {
                    "list_class.html": {
                      "ref": "list_class.html",
                      "tf": 635
                    },
                    "AutomaticForm.html": {
                      "ref": "AutomaticForm.html",
                      "tf": 110
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "r": {
              "docs": {
                "AutomaticForm.html": {
                  "ref": "AutomaticForm.html",
                  "tf": 0.7936507936507936
                }
              }
            }
          },
          "r": {
            "docs": {},
            "e": {
              "docs": {},
              "a": {
                "docs": {},
                "t": {
                  "docs": {
                    "AutomaticForm.html": {
                      "ref": "AutomaticForm.html",
                      "tf": 0.7936507936507936
                    }
                  }
                }
              }
            },
            "o": {
              "docs": {},
              "s": {
                "docs": {},
                "s": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "r": {
                      "docs": {},
                      "i": {
                        "docs": {},
                        "g": {
                          "docs": {},
                          "i": {
                            "docs": {},
                            "n": {
                              "docs": {
                                "AutomaticForm.html": {
                                  "ref": "AutomaticForm.html",
                                  "tf": 0.7936507936507936
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "l": {
          "docs": {},
          "i": {
            "docs": {},
            "s": {
              "docs": {},
              "t": {
                "docs": {
                  "list_class.html": {
                    "ref": "list_class.html",
                    "tf": 110
                  }
                },
                ":": {
                  "docs": {},
                  "c": {
                    "docs": {},
                    "l": {
                      "docs": {},
                      "a": {
                        "docs": {},
                        "s": {
                          "docs": {},
                          "s": {
                            "docs": {
                              "list_class.html": {
                                "ref": "list_class.html",
                                "tf": 1300
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "g": {
              "docs": {},
              "a": {
                "docs": {},
                "c": {
                  "docs": {},
                  "i": {
                    "docs": {
                      "AutomaticForm.html": {
                        "ref": "AutomaticForm.html",
                        "tf": 0.7936507936507936
                      }
                    }
                  }
                }
              },
              "c": {
                "docs": {},
                "i": {
                  "docs": {
                    "AutomaticForm.html": {
                      "ref": "AutomaticForm.html",
                      "tf": 0.7936507936507936
                    }
                  }
                }
              }
            }
          }
        },
        "b": {
          "docs": {},
          "i": {
            "docs": {},
            "n": {
              "docs": {},
              "d": {
                "docs": {
                  "AutomaticForm.html": {
                    "ref": "AutomaticForm.html",
                    "tf": 1.5873015873015872
                  },
                  "AutomaticForm.html#remove": {
                    "ref": "AutomaticForm.html#remove",
                    "tf": 16.666666666666664
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "t": {
              "docs": {},
              "t": {
                "docs": {},
                "o": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "AutomaticForm.html": {
                        "ref": "AutomaticForm.html",
                        "tf": 1.5873015873015872
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "r": {
              "docs": {
                "AutomaticForm.html#showProgressBar": {
                  "ref": "AutomaticForm.html#showProgressBar",
                  "tf": 16.666666666666664
                }
              }
            }
          }
        },
        "e": {
          "docs": {},
          "l": {
            "docs": {},
            "e": {
              "docs": {},
              "m": {
                "docs": {},
                "e": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "AutomaticForm.html": {
                          "ref": "AutomaticForm.html",
                          "tf": 0.7936507936507936
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "n": {
            "docs": {},
            "a": {
              "docs": {},
              "b": {
                "docs": {},
                "l": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "/": {
                      "docs": {},
                      "d": {
                        "docs": {},
                        "i": {
                          "docs": {},
                          "s": {
                            "docs": {
                              "AutomaticForm.html#showProgressBar": {
                                "ref": "AutomaticForm.html#showProgressBar",
                                "tf": 16.666666666666664
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "f": {
          "docs": {},
          "e": {
            "docs": {},
            "t": {
              "docs": {},
              "c": {
                "docs": {},
                "h": {
                  "docs": {
                    "AutomaticForm.html": {
                      "ref": "AutomaticForm.html",
                      "tf": 1.5873015873015872
                    }
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "e": {
              "docs": {},
              "l": {
                "docs": {},
                "d": {
                  "docs": {
                    "AutomaticForm.html": {
                      "ref": "AutomaticForm.html",
                      "tf": 0.7936507936507936
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "r": {
              "docs": {},
              "m": {
                "docs": {
                  "AutomaticForm.html": {
                    "ref": "AutomaticForm.html",
                    "tf": 2.380952380952381
                  },
                  "AutomaticForm.html#showMessage": {
                    "ref": "AutomaticForm.html#showMessage",
                    "tf": 12.5
                  },
                  "AutomaticForm.html#validate": {
                    "ref": "AutomaticForm.html#validate",
                    "tf": 25
                  },
                  "AutomaticForm.html#isValid": {
                    "ref": "AutomaticForm.html#isValid",
                    "tf": 25
                  },
                  "AutomaticForm.html#render": {
                    "ref": "AutomaticForm.html#render",
                    "tf": 25
                  },
                  "AutomaticForm.html#reset": {
                    "ref": "AutomaticForm.html#reset",
                    "tf": 25
                  },
                  "AutomaticForm.html#populate": {
                    "ref": "AutomaticForm.html#populate",
                    "tf": 25
                  },
                  "AutomaticForm.html#remove": {
                    "ref": "AutomaticForm.html#remove",
                    "tf": 16.666666666666664
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "n": {
              "docs": {},
              "c": {
                "docs": {},
                "t": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "n": {
                        "docs": {
                          "AutomaticForm.html": {
                            "ref": "AutomaticForm.html",
                            "tf": 1.5873015873015872
                          },
                          "AutomaticForm.html#_retrieveSchema": {
                            "ref": "AutomaticForm.html#_retrieveSchema",
                            "tf": 110
                          },
                          "AutomaticForm.html#setURI": {
                            "ref": "AutomaticForm.html#setURI",
                            "tf": 110
                          },
                          "AutomaticForm.html#setSchema": {
                            "ref": "AutomaticForm.html#setSchema",
                            "tf": 110
                          },
                          "AutomaticForm.html#showProgressBar": {
                            "ref": "AutomaticForm.html#showProgressBar",
                            "tf": 110
                          },
                          "AutomaticForm.html#showMessage": {
                            "ref": "AutomaticForm.html#showMessage",
                            "tf": 110
                          },
                          "AutomaticForm.html#validate": {
                            "ref": "AutomaticForm.html#validate",
                            "tf": 110
                          },
                          "AutomaticForm.html#isValid": {
                            "ref": "AutomaticForm.html#isValid",
                            "tf": 110
                          },
                          "AutomaticForm.html#render": {
                            "ref": "AutomaticForm.html#render",
                            "tf": 110
                          },
                          "AutomaticForm.html#reset": {
                            "ref": "AutomaticForm.html#reset",
                            "tf": 110
                          },
                          "AutomaticForm.html#populate": {
                            "ref": "AutomaticForm.html#populate",
                            "tf": 110
                          },
                          "AutomaticForm.html#remove": {
                            "ref": "AutomaticForm.html#remove",
                            "tf": 110
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "h": {
          "docs": {},
          "i": {
            "docs": {},
            "d": {
              "docs": {},
              "d": {
                "docs": {},
                "e": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "AutomaticForm.html": {
                        "ref": "AutomaticForm.html",
                        "tf": 0.7936507936507936
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "j": {
          "docs": {},
          "s": {
            "docs": {},
            "o": {
              "docs": {},
              "n": {
                "docs": {
                  "AutomaticForm.html": {
                    "ref": "AutomaticForm.html",
                    "tf": 1.5873015873015872
                  }
                }
              }
            }
          }
        },
        "m": {
          "docs": {},
          "a": {
            "docs": {},
            "r": {
              "docs": {},
              "k": {
                "docs": {},
                "u": {
                  "docs": {},
                  "p": {
                    "docs": {
                      "AutomaticForm.html": {
                        "ref": "AutomaticForm.html",
                        "tf": 0.7936507936507936
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "d": {
              "docs": {},
              "e": {
                "docs": {},
                "l": {
                  "docs": {
                    "AutomaticForm.html": {
                      "ref": "AutomaticForm.html",
                      "tf": 0.7936507936507936
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "s": {
              "docs": {},
              "s": {
                "docs": {},
                "a": {
                  "docs": {},
                  "g": {
                    "docs": {
                      "AutomaticForm.html#showMessage": {
                        "ref": "AutomaticForm.html#showMessage",
                        "tf": 62.5
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "o": {
          "docs": {},
          "p": {
            "docs": {},
            "t": {
              "docs": {},
              "i": {
                "docs": {},
                "o": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "AutomaticForm.html": {
                        "ref": "AutomaticForm.html",
                        "tf": 0.7936507936507936
                      }
                    }
                  }
                }
              }
            }
          },
          "t": {
            "docs": {},
            "h": {
              "docs": {},
              "e": {
                "docs": {},
                "r": {
                  "docs": {
                    "AutomaticForm.html": {
                      "ref": "AutomaticForm.html",
                      "tf": 0.7936507936507936
                    }
                  }
                }
              }
            }
          }
        },
        "t": {
          "docs": {},
          "i": {
            "docs": {},
            "t": {
              "docs": {},
              "l": {
                "docs": {
                  "AutomaticForm.html": {
                    "ref": "AutomaticForm.html",
                    "tf": 1.5873015873015872
                  }
                }
              }
            }
          }
        },
        "u": {
          "docs": {},
          "i": {
            "docs": {
              "AutomaticForm.html": {
                "ref": "AutomaticForm.html",
                "tf": 0.7936507936507936
              }
            }
          },
          "r": {
            "docs": {},
            "i": {
              "docs": {
                "AutomaticForm.html": {
                  "ref": "AutomaticForm.html",
                  "tf": 1.5873015873015872
                },
                "AutomaticForm.html#setURI": {
                  "ref": "AutomaticForm.html#setURI",
                  "tf": 75
                }
              }
            }
          },
          "s": {
            "docs": {
              "AutomaticForm.html": {
                "ref": "AutomaticForm.html",
                "tf": 0.7936507936507936
              }
            }
          }
        },
        "v": {
          "docs": {},
          "a": {
            "docs": {},
            "l": {
              "docs": {},
              "i": {
                "docs": {},
                "d": {
                  "docs": {
                    "AutomaticForm.html": {
                      "ref": "AutomaticForm.html",
                      "tf": 0.7936507936507936
                    },
                    "AutomaticForm.html#validate": {
                      "ref": "AutomaticForm.html#validate",
                      "tf": 775
                    },
                    "AutomaticForm.html#isValid": {
                      "ref": "AutomaticForm.html#isValid",
                      "tf": 25
                    }
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "e": {
              "docs": {},
              "w": {
                "docs": {
                  "AutomaticForm.html": {
                    "ref": "AutomaticForm.html",
                    "tf": 0.7936507936507936
                  }
                }
              }
            }
          }
        },
        "_": {
          "docs": {},
          "r": {
            "docs": {},
            "e": {
              "docs": {},
              "t": {
                "docs": {},
                "r": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "e": {
                      "docs": {},
                      "v": {
                        "docs": {},
                        "e": {
                          "docs": {},
                          "s": {
                            "docs": {},
                            "c": {
                              "docs": {},
                              "h": {
                                "docs": {},
                                "e": {
                                  "docs": {},
                                  "m": {
                                    "docs": {},
                                    "a": {
                                      "docs": {
                                        "AutomaticForm.html#_retrieveSchema": {
                                          "ref": "AutomaticForm.html#_retrieveSchema",
                                          "tf": 750
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        }
      },
      "length": 112
    },
    "corpusTokens": [
      "_retrieveschema",
      "add",
      "addit",
      "array",
      "augmented.j",
      "automat",
      "automaticform",
      "automaticform#_retrieveschema",
      "automaticform#isvalid",
      "automaticform#popul",
      "automaticform#remov",
      "automaticform#rend",
      "automaticform#reset",
      "automaticform#setschema",
      "automaticform#seturi",
      "automaticform#showmessag",
      "automaticform#showprogressbar",
      "automaticform#valid",
      "bar",
      "bind",
      "button",
      "class",
      "cor",
      "creat",
      "crossorigin",
      "data",
      "descript",
      "develop",
      "display",
      "document",
      "dom",
      "element",
      "enable/dis",
      "fetch",
      "field",
      "form",
      "function",
      "global",
      "hidden",
      "index",
      "isvalid",
      "json",
      "legaci",
      "legci",
      "list",
      "list:class",
      "markup",
      "messag",
      "model",
      "name",
      "nestedinput",
      "next",
      "option",
      "other",
      "popul",
      "prepopul",
      "present",
      "progress",
      "readm",
      "relat",
      "remov",
      "render",
      "reset",
      "resetbutton",
      "schema",
      "set",
      "setschema",
      "seturi",
      "show",
      "showmessag",
      "showprogressbar",
      "simpli",
      "submit",
      "submitbutton",
      "support",
      "titl",
      "ui",
      "uri",
      "us",
      "valid",
      "view"
    ],
    "pipeline": [
      "trimmer",
      "stopWordFilter",
      "stemmer"
    ]
  },
  "store": {
    "index.html": {
      "id": "index.html",
      "kind": "readme",
      "title": "Augmented.js Next Presentation",
      "longname": "index",
      "name": "Augmented.js Next Presentation",
      "tags": "index",
      "summary": "Augmented.js Next Presentation - Simply Development.",
      "description": "",
      "body": ""
    },
    "global.html": {
      "id": "global.html",
      "kind": "global",
      "title": "Globals",
      "longname": "global",
      "name": "Globals",
      "tags": "global",
      "summary": "All documented globals.",
      "description": "",
      "body": ""
    },
    "list_class.html": {
      "id": "list_class.html",
      "kind": "list",
      "title": "Classes",
      "longname": "list:class",
      "name": "Classes",
      "tags": "list:class",
      "summary": "All documented classes.",
      "description": "",
      "body": ""
    },
    "AutomaticForm.html": {
      "id": "AutomaticForm.html",
      "kind": "class",
      "title": "AutomaticForm",
      "longname": "AutomaticForm",
      "name": "AutomaticForm",
      "tags": "AutomaticForm",
      "summary": "",
      "description": "A automatic form view created from a JSON Schema Supported options: schema - The JSON Schema for use with the UI and validation data - prepopulate the model data crossOrigin - Set CORS for the fetch uri - the uri to fetch data from title - the title of the form description display - Array of fields to display (others are hidden) nestedInput - add additional markup in the form (DOM elements) submitButton - set the name of the submit button (binds to a submit function) resetButton - set the name of the reset button (binds to a reset function) legacy - set legcy rendering",
      "body": ""
    },
    "AutomaticForm.html#_retrieveSchema": {
      "id": "AutomaticForm.html#_retrieveSchema",
      "kind": "function",
      "title": "_retrieveSchema()",
      "longname": "AutomaticForm#_retrieveSchema",
      "name": "_retrieveSchema",
      "tags": "AutomaticForm#_retrieveSchema _retrieveSchema",
      "summary": "",
      "description": ""
    },
    "AutomaticForm.html#setURI": {
      "id": "AutomaticForm.html#setURI",
      "kind": "function",
      "title": "setURI( uri )",
      "longname": "AutomaticForm#setURI",
      "name": "setURI",
      "tags": "AutomaticForm#setURI setURI",
      "summary": "",
      "description": "Sets the URI"
    },
    "AutomaticForm.html#setSchema": {
      "id": "AutomaticForm.html#setSchema",
      "kind": "function",
      "title": "setSchema( schema )",
      "longname": "AutomaticForm#setSchema",
      "name": "setSchema",
      "tags": "AutomaticForm#setSchema setSchema",
      "summary": "",
      "description": "Sets the schema"
    },
    "AutomaticForm.html#showProgressBar": {
      "id": "AutomaticForm.html#showProgressBar",
      "kind": "function",
      "title": "showProgressBar( show )",
      "longname": "AutomaticForm#showProgressBar",
      "name": "showProgressBar",
      "tags": "AutomaticForm#showProgressBar showProgressBar",
      "summary": "",
      "description": "Enable/Disable the progress bar"
    },
    "AutomaticForm.html#showMessage": {
      "id": "AutomaticForm.html#showMessage",
      "kind": "function",
      "title": "showMessage( message )",
      "longname": "AutomaticForm#showMessage",
      "name": "showMessage",
      "tags": "AutomaticForm#showMessage showMessage",
      "summary": "",
      "description": "Show a message related to the form"
    },
    "AutomaticForm.html#validate": {
      "id": "AutomaticForm.html#validate",
      "kind": "function",
      "title": "validate()",
      "longname": "AutomaticForm#validate",
      "name": "validate",
      "tags": "AutomaticForm#validate validate",
      "summary": "",
      "description": "Validate the form"
    },
    "AutomaticForm.html#isValid": {
      "id": "AutomaticForm.html#isValid",
      "kind": "function",
      "title": "isValid()",
      "longname": "AutomaticForm#isValid",
      "name": "isValid",
      "tags": "AutomaticForm#isValid isValid",
      "summary": "",
      "description": "Is the form valid"
    },
    "AutomaticForm.html#render": {
      "id": "AutomaticForm.html#render",
      "kind": "function",
      "title": "render()",
      "longname": "AutomaticForm#render",
      "name": "render",
      "tags": "AutomaticForm#render render",
      "summary": "",
      "description": "Render the form"
    },
    "AutomaticForm.html#reset": {
      "id": "AutomaticForm.html#reset",
      "kind": "function",
      "title": "reset()",
      "longname": "AutomaticForm#reset",
      "name": "reset",
      "tags": "AutomaticForm#reset reset",
      "summary": "",
      "description": "Reset the form"
    },
    "AutomaticForm.html#populate": {
      "id": "AutomaticForm.html#populate",
      "kind": "function",
      "title": "populate( data )",
      "longname": "AutomaticForm#populate",
      "name": "populate",
      "tags": "AutomaticForm#populate populate",
      "summary": "",
      "description": "Populate the form"
    },
    "AutomaticForm.html#remove": {
      "id": "AutomaticForm.html#remove",
      "kind": "function",
      "title": "remove()",
      "longname": "AutomaticForm#remove",
      "name": "remove",
      "tags": "AutomaticForm#remove remove",
      "summary": "",
      "description": "Remove the form and all binds"
    }
  }
};